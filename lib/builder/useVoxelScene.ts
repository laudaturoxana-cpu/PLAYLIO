...
const UNDO_LIMIT = 20   // max undo steps
...
export function useVoxelScene(userId: string, sceneId: string) {
  ...
  const undoStack   = useRef<VoxelState[]>([])
  ...
  function pushUndo(snapshot: VoxelState) {
    undoStack.current = [...undoStack.current.slice(-UNDO_LIMIT + 1), snapshot]
  }
