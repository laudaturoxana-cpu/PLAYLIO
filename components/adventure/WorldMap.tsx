...
export default function WorldMap({ playerLevel, profileName, savedStarsMap }: WorldMapProps) {
  ...
  return (
    <div className="game-container min-h-screen px-4 py-6">
      ...
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="text-center">
          ...
          <p className="font-nunito text-xs text-[var(--gray)]">
            {collectedAll}/{totalStarsAll} stars collected
          </p>
        </div>
        <div className="w-10" />
      </div>
      ...
      {/* Lio message with global near-miss */}
      <div className="mb-5 flex items-center gap-3 rounded-2xl bg-white p-4 shadow-sm border border-black/5">
        ...
        <p className="font-nunito text-sm text-[var(--dark)]">
          {collectedAll === 0
            ? `${profileName}, adventure awaits! Choose a zone! 🗺️`
            : collectedAll === totalStarsAll
            ? `${profileName}, you explored EVERYTHING! You're a legendary hero! 🏆`
            : `${profileName}, you have ${collectedAll} stars! Continue the adventure! ⭐`}
        </p>
      </div>
